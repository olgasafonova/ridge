...
**Named standing violation — resolved 27-08-2026.** The `serverInstructions` block in `cmd/ridge/main.go` was stale (three languages claimed of six substrates, seven formats listed of nine); it is now derived at startup from the live registries via `tools.SupportedLanguages()` and `tools.SupportedFormats()`, with `TestServerInstructionsCoverRegistries` (`cmd/ridge/main_test.go`) pinning the wiring.
